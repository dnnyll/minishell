...
- ✅ Variable Expansion:
    - $VAR → expanded using env/export list
...
echo $USER > $OUT       ➜ argv: ["echo", "john"], outfile: "output.txt"
cat <<EOF | grep hi     ➜ heredoc_delim: "EOF", next: grep command
ls -l | wc -l           ➜ two commands linked by `next`
